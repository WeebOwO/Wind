...
namespace wind
{
    ...
    private:
        ...
        wind::vector<std::unique_ptr<JobBuffer>> m_jobBuffers;
    };
...
namespace wind
{
    ...
    void JobSystem::RunJob(const JobDeclaration& job)
    {
        // run a single job
    ...
    JobCounter* JobSystem::AllocateCounter()
    {
        // allocate a counter
    ...
    void JobSystem::WaitAndFreeCounter(JobCounter* counter)
    {
        counter->Wait();
        std::lock_guard<std::mutex> lock(s_allocCounterMutex);
